...
def parse_args(
    ...
    parser.add_argument('--max_iter',type=int,default=400000,help='maximum iteration to train')
    ...
    parser.add_argument('--save_dir',type=str,required=True,help='output save directory')
    if callback is not None:
        parser = callback(parser)
    args = parser.parse_args()

    if args.exp_group_name is not None:
        args.logs_path = os.path.join(args.exp_group_name,args.logs_path)
        args.checkpoints_path = os.path.join(args.exp_group_name,args.checkpoints_path)
        args.visual_path = os.path.join(args.exp_group_name,args.visual_path)

    os.makedirs(os.path.join(args.save_dir,args.name,args.checkpoints_path), exist_ok=True)
    os.makedirs(os.path.join(args.save_dir,args.name,args.visual_path), exist_ok=True)
