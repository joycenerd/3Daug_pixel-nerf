...
class PixelNeRFNet(torch.nn.Module):
    ...
    def load_weights(self, args, opt_init=False, strict=True, device=None):
        ...
        ckpt_name = (
            ...
            "pixel_nerf_init" if opt_init or not args.resume else "pixel_nerf_backup"
        )
        model_path = "%s/%s/%s/%s" % (args.save_dir,args.name,args.checkpoints_path, ckpt_name)
    ...
    def save_weights(self, args, opt_init=False):
        ...
        ckpt_path = osp.join(args.save_dir,args.name,args.checkpoints_path, ckpt_name)
        ckpt_backup_path = osp.join(args.save_dir,args.name,args.checkpoints_path, backup_name)
