...
def import_object(model_dir, model_path, axis_forward="-Z", axis_up="Y"):
    ...
    path=model_dir
    print(f'import_object path: {path}')
...
def setup_scene(args, model_objs):
...
def _move_files(dirname, file_prefixes):
    ...
    for prefix in file_prefixes:
        ...
        print(f'_move_files output: {output}')
...
def render_views(
    ...
    print(f'render_views args.out_dir: {args.out_dir}')
    model_obj=model_dirs[0].split('\\')[-1]
    print(f'model_dirs[0]: {model_dirs[0]}')
    print(f'model_obj: {model_obj}')    
    out_dir=osp.join(args.out_dir,model_obj.split('.')[0])
    print(f'render_views out_dir: {out_dir}')
    ...
    for i in range(args.n_views):
        ...
        print(f'render_views filepath: {filepath}')
        files.extend(_render_single(filepath, camera, args))
        print('_render_single completed...')
    ...
    print(f"render_views transform path: {osp.join(out_dir, 'transforms.json')}")
    delta = time() - start
...
def get_split(args):
    ...
    # ]
    models_all=glob.glob(f'{object_dir}/*.obj')
...
def parse_args():
    ...
    #     args.out_dir, args.split,"{}_{}obj".format(obj_id, args.n_objects)
    # )
    args.out_dir=osp.join(args.out_dir,args.split)
    print(args)
    return args
